...
# CR_hourly <- pad(CR_hourly, by = "hour")
#   ag_df_CR <- pad(morndf_CR)
# morndf_CR <- pad(morndf_CR)
...
process_ARIMA <- function(data, variable, start_year, start_period, frequency, label) {
  print(paste("Processing:", label, variable))
  
  data[[variable]] <- as.numeric(data[[variable]])
  
  # Check for date column
  if (!"date" %in% names(data)) {
    stop("Dataframe must have a date column named 'date'")
  }
  
  if (label == "hourly") {
    data <- data %>%
      mutate(datetime = as.POSIXct(date)) %>%
      mutate(datetime = lubridate::floor_date(datetime, unit = "hour")) %>%
  ...
  data <- na.omit(data)
  print(paste("Rows after padding and na.omit:", nrow(data)))
  
  if(nrow(data) < 10) {
    warning(paste("Not enough data after padding and na.omit for", label, variable))
    return(tibble())
  }
  
  ts_data <- ts(data[[variable]], start = c(start_year, start_period), frequency = frequency)
  fit <- auto.arima(ts_data, seasonal = TRUE)
  fcast <- forecast(fit, h = frequency)
  # saveRDS(fcast, file = paste0("ARIMA_", label, "_", variable, ".rds"))
  
  aic_val <- AIC(fit)
  rmse_val <- accuracy(fcast)[2]
  
  residuals_output <- capture.output(checkresiduals(fit))
  print(residuals_output)  # Debug print to see exact output
  
   model_line <- residuals_output[4] %>% 
  str_remove("^data:\\s*") %>% 
  str_trim() %>%
  str_remove("^Residuals from\\s*")
print(model_line)

q_star <- residuals_output[5] %>% 
  str_match("Q\\* = ([0-9\\.]+)") %>% 
  .[2] %>% 
  as.numeric()
print(q_star)
...
p_val <- str_match(residuals_output, "p-value\\s*(<|=)\\s*([0-9\\.e\\-]+)") %>% 
  .[5,3] %>% 
  as.numeric()
print(p_val)
...
# Safe wrapper to catch errors and always return a tibble
safe_process_ARIMA <- function(...) {
  tryCatch(
    process_ARIMA(...),
    error = function(e) {
      warning(paste("Error in process_ARIMA:", e$message))
      return(tibble())  # empty tibble on error
    }
  )
}

# Run ARIMA processing safely for all datasets/variables
results <- list(
  safe_process_ARIMA(morndf_CR, "chl", 2015, 5, 365, "morndf"),
  safe_process_ARIMA(morndf_CR, "phy", 2015, 5, 365, "morndf"),
  safe_process_ARIMA(ag_df_CR, "chl", 2015, 5, 365, "daily"),
  safe_process_ARIMA(ag_df_CR, "phy", 2015, 5, 365, "daily")
  # Add your hourly calls if data available and ready:
  # safe_process_ARIMA(CR_hourly, "chl", 2015, 5, 8760, "hourly"),
  # safe_process_ARIMA(CR_hourly, "phy", 2015, 5, 8760, "hourly")
)

# Remove any empty results and combine
...
image_path <- "C:/Users/mfman/OneDrive/Desktop/HAB2/HAB2/3_OUTPUT/4_5_figures_and_tables"
...
image_path <- "C:/Users/mfman/OneDrive/Desktop/HAB2/HAB2/3_OUTPUT/4_5_figures_and_tables/ARIMA.png"


# Convert data frame to table grob
table_grob <- tableGrob(ARIMA)

# Save as PNG
ggsave(image_path, table_grob, width = 10, height = 5, dpi = 300)
...
for (dataset_name in names(data)) {
  ...
  for (var in variables) {
    ts_data <- ts(df[[var]])  # Optionally add start, frequency if needed
    
    
    key <- paste(dataset_name, var, sep = "_")
    results_list[[key]] <- ts_data
    
    # Fit ARIMA model
    fit <- auto.arima(ts_data, seasonal = TRUE)
    
    # Forecast next 365 periods (assumed daily)
    fcast <- forecast(fit, h = length(data))

    # Store forecast in dataframe
    ARIMA_pred <- data.frame(forecast = as.numeric(fcast$mean))
  
          # Create date sequence starting from a specific date
    start_date <- as.Date("2023-01-01") + 171  # 172nd day of year
  
      ARIMA_pred$date <- seq(from = start_date, by = "day", length.out = nrow(ARIMA_pred))
    data$date <- seq(from = start_date
                     
                     
                     , by = "day", length.out = nrow(data))

    
    # Print result
    print(paste("Forecast for:", key))
    print(head(ARIMA_pred))
